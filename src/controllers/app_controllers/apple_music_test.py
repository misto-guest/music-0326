...
class AppleMusicController(BaseController, PopupMonitorMixin):
    ...
    def _handle_shuffle_and_miniplayer(self) -> bool:
        """Improved version with multiple shuffle button selectors and fallbacks."""
        try:
            if not self._verify_rotation_disabled():
                return False

            # Multiple selectors for shuffle button - try different approaches
            shuffle_selectors = [
                '//*[@resource-id="com.apple.android.music:id/button_shuffle"]',
                '//*[contains(@resource-id, "shuffle")]',
                '//*[@content-desc="Shuffle" or @text="Shuffle"]',
                '//*[@content-desc="Shuffle All" or @text="Shuffle All"]',
                '//*[contains(@content-desc, "shuffle") or contains(@text, "shuffle")]',
            ]

            shuffle_clicked = False
            for selector in shuffle_selectors:
                try:
                    shuffle_button = self.device.xpath(selector)
                    if shuffle_button.exists:
                        logger.info(f"Found shuffle button with selector: {selector}")
            ...
            if not shuffle_clicked:
                # Try finding any clickable element that might be shuffle
                logger.warning("No shuffle button found via selectors, searching for clickable elements...")
                try:
                    # Look for buttons in the current view
                    clickable_shuffle = self.device.xpath(
                        '//*[contains(@text, "shuffle") or contains(@content-desc, "shuffle")]'
                    )
                    if clickable_shuffle.exists:
                        clickable_shuffle.click()
                        logger.info("Clicked shuffle via text search")
                        shuffle_clicked = True
                except Exception as e:
                    logger.debug(f"Text search failed: {e}")

            if not shuffle_clicked:
                # Last resort: use keyevent to start playback
                logger.warning("Shuffle button not found, using keyevent fallback to start playback")
                self.device.shell('input keyevent KEYCODE_MEDIA_PLAY')
                time.sleep(2)
                # Still consider this success if we can get playback going
                shuffle_clicked = True
            ...
            miniplayer_selectors = [
                ...
                '//*[contains(@resource-id, "mini_player")]',
            ]
            ...
            logger.warning("Miniplayer not found after trying multiple selectors, but shuffle may have worked")
            # Return True anyway if shuffle was clicked - miniplayer is not critical
            return shuffle_clicked
        except Exception as e:
